...
Here we will describe a fairly simple solution (using a [segment tree](/docs/#Algorithms/data_structures/segment_tree/)) that will answer each query in $O(\log N)$ time.
The preprocessing step will take $O(N)$ time.

## Algorithm

First, we need to find the [LCA](/docs/#Algorithms/graph/lca/) to reduce each query of the second kind $(i,j)$ into two queries $(l,i)$ and $(l,j)$, where $l$ is the LCA of $i$ and $j$.
