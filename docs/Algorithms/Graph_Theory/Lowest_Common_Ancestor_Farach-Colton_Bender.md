...
We traverse all nodes of the tree with [DFS](/docs/#Algorithms/graph/depth-first-search/) and keep an array with all visited nodes and the heights of these nodes.
...
You can read more about this reduction in the article [Lowest Common Ancestor](/docs/#Algorithms/graph/lca/).
...
There is an easy data structure that we can use for solving the RMQ problem with $O(N \log N)$ preprocessing and $O(1)$ for each query: the [Sparse Table](/docs/#Algorithms/data_structures/sparse-table/).
