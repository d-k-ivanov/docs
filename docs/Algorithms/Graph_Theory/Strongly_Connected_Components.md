...
Described algorithm was independently suggested by Kosaraju and Sharir at 1979. This is an easy-to-implement algorithm based on two series of [depth first search](/docs/#Algorithms/Graph_Theory/Depth_First_Search/), and working for $O(n + m)$ time.
...
Finally, it is appropriate to mention [topological sort](/docs/#Algorithms/Graph_Theory/Topological_Sorting/) here. First of all, step 1 of the algorithm represents reversed topological sort of graph $G$ (actually this is exactly what vertices' sort by exit time means). Secondly, the algorithm's scheme generates strongly connected components by decreasing order of their exit times, thus it generates components - vertices of condensation graph - in topological sort order.
