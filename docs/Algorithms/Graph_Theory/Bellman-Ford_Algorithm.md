...
For more on this topic — see separate article, [Finding a negative cycle in the graph](/docs/#Algorithms/graph/finding-negative-cycle-in-graph/).
...
See also the problem list in the article [Finding the negative cycle in a graph](/docs/#Algorithms/graph/finding-negative-cycle-in-graph.html).
