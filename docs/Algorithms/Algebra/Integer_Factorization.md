...
The idea comes from [Fermat's little theorem](/docs/#Algorithms/Algebra/Euler_Totient_Function/#application).
...
Therefore, if $p - 1$ for a factor $p$ of $n$ divides $M$, we can extract a factor using [Euclid's algorithm](/docs/#Algorithms/Algebra/GCD_Euclidean_Algorithm/).
