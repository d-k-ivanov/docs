...
**Second proof**: Note that if mask $m$ has $k$ enabled bits, then it will have $2^k$ submasks. As we have a total of $\binom{n}{k}$ masks with $k$ enabled bits (see [binomial coefficients](/docs/#Algorithms/Combinatorics/Binomial_Coefficients/)), then the total number of combinations for all masks will be:
