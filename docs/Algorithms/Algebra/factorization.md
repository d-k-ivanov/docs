...
So it makes sense to perform a probabilistic (or a fast deterministic) [primality test](/docs/#Algorithms/algebra/primality_tests/) before trying to factorize the number.
...
So a good way of checking is just to precompute all prime numbers with the [Sieve of Eratosthenes](/docs/#Algorithms/algebra/sieve-of-eratosthenes/) until $\sqrt{n}$ and test them individually.
...
The idea comes from [Fermat's little theorem](/docs/#Algorithms/algebra/phi-function/#application).
...
Therefore, if $p - 1$ for a factor $p$ of $n$ divides $M$, we can extract a factor using [Euclid's algorithm](/docs/#Algorithms/algebra/euclid-algorithm/).
...
If GCC is not available, you can using a similar idea as [binary exponentiation](/docs/#Algorithms/algebra/binary-exp/).
...
Alternatively you can also implement the [Montgomery multiplication](/docs/#Algorithms/algebra/montgomery_multiplication/).
