...
See for instance the article about the [Sparse Table](/docs/#Algorithms/Sata_Structures/Sparse_Table/), which uses a similar idea and computes all $\log$ values.
...
Thus if we have such an array $\text{lcp}$, then the problem is reduced to the [RMQ](/docs/#Algorithms/Sequences/Range_Minimum_Query/), which has many wide number of different solutions with different complexities.
