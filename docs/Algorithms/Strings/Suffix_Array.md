...
See for instance the article about the [Sparse Table](/docs/#Algorithms/data_structures/sparse-table/), which uses a similar idea and computes all $\log$ values.
...
Thus if we have such an array $\text{lcp}$, then the problem is reduced to the [RMQ](/docs/#Algorithms/sequences/rmq/), which has many wide number of different solutions with different complexities.
