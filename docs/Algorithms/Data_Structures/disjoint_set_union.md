...
![Example-image of the set representation with trees](/docs/data/cs/DSU_example.png)
...
![Path compression of call `find_set(7)`](/docs/data/cs/DSU_path_compression.png)
...
This application is quite important, because nearly the same problem appears in [Kruskal's algorithm for finding a minimum spanning tree](/docs/#Algorithms/Graph_Theory/Minimum_Spanning_Tree_Kruskals_Algorithm/).
Using DSU we can [improve](/docs/#Algorithms/Graph_Theory/Minimum_Spanning_Tree_Kruskal_with_DSU/) the $O(m \log n + n^2)$ complexity to $O(m \log n)$.
...
The problem can also be solved by [DFS](/docs/#Algorithms/Graph_Theory/Depth_First_Search/) or [BFS](/docs/#Algorithms/Graph_Theory/Breadth-First_Search/), but the method described here has an advantage:
...
The algorithm for finding the LCA is discussed in the article [Lowest Common Ancestor - Tarjan's off-line algorithm](/docs/#Algorithms/Graph_Theory/Lowest_Common_Ancestor_Tarjan/).
This algorithm compares favorable with other algorithms for finding the LCA due to its simplicity (especially compared to an optimal algorithm like the one from [Farach-Colton and Bender](/docs/#Algorithms/Graph_Theory/Lowest_Common_Ancestor_Farach-Colton_Bender/)).
...
we can implement a [DFS](/docs/#Algorithms/Graph_Theory/Depth_First_Search/), which will return a pointer to a set of integers - the list of numbers in that subtree.
...
More details (including proof of the time complexity) can be found in the article [Finding Bridges Online](/docs/#Algorithms/Graph_Theory/Bridge_Searching_Online/).
