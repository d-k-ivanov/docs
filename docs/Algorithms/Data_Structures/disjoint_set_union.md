...
This application is quite important, because nearly the same problem appears in [Kruskal's algorithm for finding a minimum spanning tree](/docs/#Algorithms/graph/mst_kruskal/).
Using DSU we can [improve](/docs/#Algorithms/graph/mst_kruskal_with_dsu/) the $O(m \log n + n^2)$ complexity to $O(m \log n)$.
...
The problem can also be solved by [DFS](/docs/#Algorithms/graph/depth-first-search/) or [BFS](/docs/#Algorithms/graph/breadth-first-search/), but the method described here has an advantage:
...
The algorithm for finding the LCA is discussed in the article [Lowest Common Ancestor - Tarjan's off-line algorithm](/docs/#Algorithms/graph/lca_tarjan/).
This algorithm compares favorable with other algorithms for finding the LCA due to its simplicity (especially compared to an optimal algorithm like the one from [Farach-Colton and Bender](/docs/#Algorithms/graph/lca_farachcoltonbender/)).
...
we can implement a [DFS](/docs/#Algorithms/graph/depth-first-search/), which will return a pointer to a set of integers - the list of numbers in that subtree.
...
More details (including proof of the time complexity) can be found in the article [Finding Bridges Online](/docs/#Algorithms/graph/bridge-searching-online/).
